...
const Bikes = () => {
    ...
    const qrCodeTemplate = (rowData: any) => {
        ...
        return (
            <div>
                <div style={{ height: 'auto', margin: '0 auto', maxWidth: 64, width: '100%' }} ref={qrcodeRefs.current[rowData.deviceId]}>
                    <QRCode size={256} style={{ height: 'auto', maxWidth: '100%', width: '100%' }} value={rowData.deviceId.toString()} viewBox={`0 0 256 256`} />
                </div>
                <Button text style={{ padding: '0px' }} onClick={downloadQRCode(qrcodeRefs.current[rowData.deviceId], rowData.deviceId.toString())}>
                    {' '}
                    Download{' '}
                </Button>
            </div>
