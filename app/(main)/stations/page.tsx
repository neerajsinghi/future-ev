...
const Stations = () => {
    ...
    const statusStockTemplate = (rowData: any) => {
        const id = rowData.id
        return <div>{rowData.stock ? <Link href={{
            pathname: '/bikesStationed',
            query: {
                search: id
            }
        }}>{rowData.stock}</Link> : 0}</div>;
    }
    ...
    const statusTemplate = (rowData: any) => {
        ...
        return <Button tooltip="Click to change status" severity={rowData.status === 'available' ? "success" : "danger"} onClick={() => changeStatus(rowData.status === 'available' ? 'unavailable' : 'available', rowData.id)}>{rowData.status}</Button>;
    }
    //public template
    const statusPublicTemplate = (rowData: any) => {
        return <Button tooltip="Click to change public status" severity={rowData.public ? "success" : "danger"} onClick={() => changePublic(!rowData.public, rowData.id)}>{rowData.public ? "Yes" : "No"}</Button>;
    }
    const columns = [
        { key: 'name', label: 'Name', _props: { scope: 'col' } },
    ...
    return (
        <>
            ...
            <Dialog header="Add Station" visible={showDialog} style={{ width: '50vw' }} onHide={() => setShowDialog(false)}>
